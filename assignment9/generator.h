...
{
    Vec3f res(rand() % 255, rand() % 255, rand() % 255);
    res *= (1 / 125.f);
    return res - Vec3f(1.f, 1.f, 1.f);
}
...
public:
    HoseGenerator(Vec3f position, float position_randomness, Vec3f velocity, float velocity_randomness) : m_position(position), m_position_randomness(position_randomness), m_velocity(velocity), m_velocity_randomness(velocity_randomness){};
